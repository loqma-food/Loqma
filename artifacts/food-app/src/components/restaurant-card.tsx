  ...
  return (
    <article className="group animate-rise-in overflow-hidden rounded-[1.45rem] border border-card-border bg-card shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-md" style={{ animationDelay: `${Math.min(index * 70, 350)}ms` }} data-testid={`card-restaurant-${restaurant.id}`}>
      <div className="relative aspect-[1.55/1] overflow-hidden bg-secondary">
         {hasImage ? <img src={restaurant.imageUrl!} alt="" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /> : <div className="relative flex h-full items-center justify-center bg-[radial-gradient(circle_at_25%_25%,hsl(var(--accent))_0_12%,transparent_13%),linear-gradient(135deg,hsl(var(--secondary)),hsl(var(--muted)))]"><UtensilArt /><span className="absolute bottom-3 rounded-full bg-card/80 px-2.5 py-1 text-[11px] font-bold text-muted-foreground">Information unavailable</span></div>}
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="truncate font-display text-[1.35rem] font-bold leading-tight" data-testid={`text-restaurant-name-${restaurant.id}`}>{restaurant.name}</h3>
             <p className="mt-1 truncate text-sm text-muted-foreground">{unavailable(restaurant.cuisine) ? 'Information unavailable' : restaurant.cuisine}</p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-2 text-xs font-semibold">
           {!unavailable(restaurant.rating) ? <span className="inline-flex items-center gap-1 text-foreground"><Star className="h-3.5 w-3.5 fill-accent text-accent-foreground" />{restaurant.rating.toFixed(1)}{restaurant.reviewCount ? <span className="font-normal text-muted-foreground">({restaurant.reviewCount})</span> : null}</span> : <span className="text-muted-foreground">Information unavailable</span>}
        ...
        <div className="mt-4 flex items-center gap-1.5 border-t border-border/70 pt-3 text-xs text-muted-foreground">
          <Clock3 className="h-3.5 w-3.5" />
           {restaurant.isOpenNow === true ? 'Open today' : restaurant.isOpenNow === false ? 'Check hours' : 'Information unavailable'}
        </div>
      </div>
    </article>
  );
