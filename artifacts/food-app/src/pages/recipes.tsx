  ...
  return <SavorlyShell showChef>
    <div className="mobile-page mx-auto max-w-6xl px-5 pb-32 md:px-8 md:pb-16">
      <section className="relative overflow-hidden rounded-[2rem] bg-sidebar px-6 py-10 text-sidebar-foreground sm:px-10 sm:py-14 md:px-14">
        ...
        <div className="relative max-w-2xl"><SectionEyebrow>Cook something good</SectionEyebrow><h1 className="max-w-xl font-display text-[clamp(3rem,9vw,6.4rem)] font-bold leading-[.88] tracking-[-.065em]">Dinner, <span className="text-primary">sorted.</span></h1><p className="mt-5 max-w-lg text-base leading-7 text-sidebar-foreground/75">Recipes with enough guidance to get you from “maybe” to “look what I made.”</p></div>
      </section>
      <section className="relative z-10 -mt-6 space-y-3 px-1">
        <SearchBox value={search} onChange={setSearch} />
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide"><button type="button" onClick={() => setFiltersOpen((open) => !open)} className="touch-target inline-flex shrink-0 items-center gap-2 rounded-full bg-primary px-4 text-sm font-bold text-primary-foreground shadow-sm transition hover:-translate-y-0.5" data-testid="button-toggle-recipe-filters"><SlidersHorizontal className="h-4 w-4" />Filters {activeCount > 0 && <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-primary-foreground px-1 text-xs text-primary">{activeCount}</span>}</button>{activeCount > 0 && <button type="button" onClick={reset} className="touch-target inline-flex shrink-0 items-center gap-1 rounded-full border border-border bg-card px-4 text-sm font-bold text-muted-foreground" data-testid="button-clear-recipe-filters">Clear <X className="h-3.5 w-3.5" /></button>}<span className="ml-auto hidden shrink-0 text-xs font-bold uppercase tracking-[.12em] text-muted-foreground sm:block">{query.data?.total ?? 0} recipes</span></div>
         {filtersOpen && <div className="animate-rise-in grid gap-3 rounded-[1.5rem] border border-border bg-card p-4 shadow-md sm:grid-cols-2 md:grid-cols-3" data-mobile-overlay="true" data-testid="panel-recipe-filters">
          ...
          <label className="space-y-1.5 text-xs font-bold text-muted-foreground"><span>Cooking time</span><select value={params.maxCookingTime ?? ''} onChange={(event) => updateParam('maxCookingTime', event.target.value ? Number(event.target.value) : undefined)} className="min-h-11 w-full rounded-xl border border-border bg-background px-3 text-sm font-semibold text-foreground outline-none focus:border-primary" data-testid="select-filter-cooking-time"><option value="">Any duration</option><option value="20">20 minutes or less</option><option value="30">30 minutes or less</option><option value="45">45 minutes or less</option><option value="60">1 hour or less</option></select></label>
          <label className="flex min-h-11 items-center gap-3 self-end rounded-xl border border-border bg-background px-3 text-sm font-semibold"><input type="checkbox" checked={params.vegetarian === true} onChange={(event) => updateParam('vegetarian', event.target.checked ? true : undefined)} className="h-5 w-5 accent-[hsl(var(--primary))]" data-testid="checkbox-filter-vegetarian" />Vegetarian</label>
          <label className="flex min-h-11 items-center gap-3 self-end rounded-xl border border-border bg-background px-3 text-sm font-semibold"><input type="checkbox" checked={params.spicy === true} onChange={(event) => updateParam('spicy', event.target.checked ? true : undefined)} className="h-5 w-5 accent-[hsl(var(--primary))]" data-testid="checkbox-filter-spicy" />Spicy</label>
           <button type="button" onClick={() => setFiltersOpen(false)} className="min-h-11 rounded-xl border border-border bg-background px-4 text-sm font-bold sm:col-span-2 md:col-span-3 md:hidden" data-mobile-overlay-close>Done</button>
        </div>}
      </section>
      <div className="mt-10">{query.isLoading ? <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3"><RecipeSkeleton /><RecipeSkeleton /><RecipeSkeleton /></div> : query.isError ? <ErrorState onRetry={() => query.refetch()} /> : results.length === 0 ? <div className="rounded-[1.5rem] border border-dashed border-primary/40 bg-accent/20 px-6 py-16 text-center" data-testid="status-recipes-empty"><h2 className="font-display text-3xl font-bold">Nothing on the menu yet.</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">Try a broader search or clear a filter. There is a good dinner hiding in here.</p><button type="button" onClick={reset} className="mt-6 min-h-11 rounded-full bg-primary px-5 text-sm font-bold text-primary-foreground" data-testid="button-empty-clear-filters">Show every recipe</button></div> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{results.map((recipe) => <RecipeCard key={recipe.recipeId} recipe={recipe} />)}</div>}</div>
    </div>
  </SavorlyShell>;
}