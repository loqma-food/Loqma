...
export default function Restaurants() {
  ...
  const params = useMemo(() => ({
    lat: Number(search.get('lat') ?? 30.0444),
    lon: Number(search.get('lon') ?? 31.2357),
    radius: 5000,
  ...
  return <SavorlyShell>
     <div className="mobile-page mx-auto max-w-6xl px-5 pb-32 md:px-8 md:pb-16">
      <div className="flex flex-wrap items-end justify-between gap-5">
        <div>
          <Link href="/eat-out" className="mb-7 inline-flex items-center gap-2 text-sm font-bold text-muted-foreground transition hover:text-foreground" data-testid="link-back-to-eat-out"><ArrowLeft className="h-4 w-4" /> {t('adjustSearch')}</Link>
          <p className="text-xs font-bold uppercase tracking-[.16em] text-primary">{t('liveNow')}</p>
          <h1 className="mt-3 font-display text-5xl font-bold leading-[.9] tracking-[-.05em] sm:text-7xl">{language === 'ar' ? 'أماكن حلوة' : 'Good places'}<br /><span className="text-primary">{language === 'ar' ? 'هنا.' : 'this way.'}</span></h1>
        </div>
      ...
      <div className={`mt-8 overflow-hidden rounded-[1.4rem] border border-border bg-card transition-all ${filtersOpen ? 'max-h-96 p-4 opacity-100 sm:p-5' : 'max-h-0 border-transparent p-0 opacity-0'}`} data-mobile-overlay={filtersOpen ? 'true' : undefined}>
        <div className="grid gap-4 sm:grid-cols-[1.5fr_1fr_auto_auto] sm:items-end">
          <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{language === 'ar' ? 'بحث' : 'Search'}<input value={query} onChange={(event) => setQuery(event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-input bg-background px-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-primary" placeholder={language === 'ar' ? 'مطبخ أو مزاج' : 'Cuisine or mood'} data-testid="input-filter-search" /></label>
          <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{language === 'ar' ? 'المطبخ' : 'Cuisine'}<select value={cuisine} onChange={(event) => setCuisine(event.target.value === 'Any cuisine' ? '' : event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-input bg-background px-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-primary" data-testid="select-cuisine">{cuisines.map((item) => <option key={item}>{item}</option>)}</select></label>
          <button onClick={() => setOpenNow((value) => !value)} className={`min-h-11 rounded-xl border px-3 text-sm font-bold transition ${openNow ? 'border-primary bg-primary/10 text-primary' : 'border-input'}`} data-testid="button-filter-open">{openNow ? t('openNow') : t('anyHours')}</button>
          <button onClick={() => setVegetarian((value) => !value)} className={`min-h-11 rounded-xl border px-3 text-sm font-bold transition ${vegetarian ? 'border-primary bg-primary/10 text-primary' : 'border-input'}`} data-testid="button-filter-vegetarian">{vegetarian ? t('vegetarian') : t('anyMenu')}</button>
          <button type="button" onClick={() => setFiltersOpen(false)} className="min-h-11 rounded-xl border border-input px-3 text-sm font-bold sm:hidden" data-mobile-overlay-close>{t('done')}</button>
        </div>
      </div>
      <div className="mt-8 flex items-center justify-between gap-4 text-sm text-muted-foreground">
         <p data-testid="text-results-summary">{isLoading ? (language === 'ar' ? 'بندور حواليك…' : 'Looking around…') : isError ? (language === 'ar' ? 'حصلت مشكلة صغيرة.' : 'We hit a small snag.') : `${results.length} ${results.length === 1 ? t('place') : t('places')}`}</p>
        <p className="hidden items-center gap-1.5 sm:flex"><LocateFixed className="h-4 w-4" /> {t('liveListings')}</p>
      </div>
      {isLoading && <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{[1, 2, 3, 4, 5, 6].map((item) => <SkeletonCard key={item} />)}</div>}
      {isError && <StatePanel title={language === 'ar' ? 'الخريطة واخدة نفس.' : 'The neighborhood map is taking a breather.'} body={language === 'ar' ? 'مش قادرين نحمّل الأماكن دلوقتي. جرّب كمان شوية.' : 'We could not load live places just now. Try once more in a moment.'} action={<button onClick={() => refetch()} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-primary px-5 text-sm font-bold text-primary-foreground" data-testid="button-retry-restaurants"><RefreshCw className="h-4 w-4" /> {t('tryAgain')}</button>} />}
      {!isLoading && !isError && results.length === 0 && <StatePanel title={language === 'ar' ? 'مفيش حاجة مطابقة.' : 'Nothing quite matched.'} body={language === 'ar' ? 'جرّب نطاق أوسع أو مطبخ تاني أو امسح الفلاتر.' : 'Try a wider radius, another cuisine, or let the filters go for a minute.'} action={<button onClick={() => { setCuisine(''); setOpenNow(false); setVegetarian(false); setQuery(''); }} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-primary px-5 text-sm font-bold text-primary-foreground" data-testid="button-clear-filters"><SlidersHorizontal className="h-4 w-4" /> {t('clear')}</button>} />}
